...
The code snippets in this plan are historical (as-written during Phase 1).
The source of truth is the current `src/` tree. Documented deviations:

- `ffi-napi`/`ref-napi` → **`koffi`** in all native tasks (rationale: spec §2.2 / AGENTS.md)
- `config.js`: synchronous `new Store()` → **async `init()`** (Task 2)
- `windows.js`: API `getWorkerW`/`setParentToWorkerW` → **`getLayout`/`attachToDesktopLayer`/`attachWallpaperWindow`/`findWallpaperWorkerW`** (raised-desktop detection, `MapWindowPoints`, physical-pixel scaling) (Task 3)
- `wallpaper-manager.create()`: `show:false` + `backgroundColor:#000000`, `type:'desktop'` only on non-Windows, HWND read via `BigUInt64LE`, `disableRoundedCorners`, pending-wallpaper buffering (`isReady`/`pendingWallpaper`) (Task 6)
- **`hotkeys.js` implemented but absent from the plan** (global shortcut `Ctrl+Shift+W`) (Task 9)
- `index.js` extras: `await config.init()`, `fs.existsSync` validation + notifications, `lw:wallpaper-error` channel, `showSettings` injected into the tray (Task 9)
- `fullscreen-detector` uses koffi (`__stdcall EnumWindows` callback) (Task 4)
...
- [x] **Step 1: Initialize npm project and install dependencies**
...
- [x] **Step 2: Write package.json scripts and main entry**
...
- [x] **Step 5: Write shared constants**
...
- [x] **Step 3: Implement Windows desktop integration**
...
- [x] **Step 3: Implement fullscreen detector**
...
- [x] **Step 1: Create wallpaper window HTML**
...
- [x] **Step 2: Create video renderer script**
...
- [x] **Step 1: Implement wallpaper manager**
...
- [x] **Step 2: Create placeholder tray icon (1x1 transparent PNG)**
...
- [x] **Step 2: Create settings app logic**
...
- [x] **Step 1: Implement main process entry**
...
*(verificado por cobertura automatizada — suítes `fullscreen-detector`)*
